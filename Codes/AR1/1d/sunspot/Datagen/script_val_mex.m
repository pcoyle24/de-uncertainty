...
for k = 1:2
    ...
    for s = length(P.sigma_grid):-1:1
        ...
        for i = 1:length(P.pi_targ)
            ...
            rss.c_s = 100*(pf.cf.c_s(inx) - 1);
            rss.inf_s = 400*(pf.cf.inf_s(inx)-1);
            rss.r_s = 400*(pf.cf.r_s(inx)-1);
            
            rss.c_d = 100*(pf.cf.c_d(inx) - 1);
            rss.inf_d = 400*(pf.cf.inf_d(inx)-1);
            rss.r_d = 400*(pf.cf.r_d(inx)-1);
            
            rss.gap.c_s = 100*(pf.cf.c_s(inx) - S.c_s);
            rss.gap.inf_s = 400*(pf.cf.inf_s(inx)-1) - 400*(S.inf_s-1);
            rss.gap.r_s = 400*(pf.cf.r_s(inx)-1) - 400*(S.r_s-1);
            
            rss.gap.c_d = 100*(pf.cf.c_d(inx) - S.c_d);
            rss.gap.inf_d = 400*(pf.cf.inf_d(inx)-1) - 400*(S.inf_d-1);
            rss.gap.r_d = 400*(pf.cf.inf_d(inx)-1) - 400*(S.r_d-1);
            
            % Save coefficients
            save(strcat(savedir,'ChebPFs_cSIGMA',num2str(100*P.sigma),'_ps',num2str(P.Ps),'_pd',num2str(P.Pd),'.mat'),'C','O','pf','S','rss');    
        end
    end
end

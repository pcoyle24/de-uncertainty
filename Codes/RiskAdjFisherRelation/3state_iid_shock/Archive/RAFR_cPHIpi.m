...
% Allocate Space
tr = zeros(numgrid,length(cPHIpi_grid));
rafr = zeros(numgrid,length(cPHIpi_grid));

for j = 1:length(cPHIpi_grid)
    cPHIpi = cPHIpi_grid(j);   
    titles = {'\phi_{\pi} = 2','\phi_{\pi} = 6','\phi_{\pi} = 10'};
...
for j = 1:length(cPHIpi_grid)
    h(j) = plot(400*pi_m, 400*tr(:,j),'Color',colors{j},'LineWidth',2);
    plot(400*pi_m, 400*rafr(:,j),'Color',colors{j},'LineStyle',':','LineWidth',2);
%     p1 = plot(400*pi_lb_zlb(j),400*rafr(inx_lb(j)));
%     p2 = plot(400*pi_ub_zlb(j),400*rafr(inx_ub(j)));
%     set([p1 p2],'Marker','o','MarkerEdgeColor','k','MarkerFaceColor', 'k','MarkerSize',8)
end

xlabel('Inflation','FontSize',16)
ylabel('Nominal Interst Rate','FontSize',16)
% L = legend([h(1) h(2) h(3)],'\phi_{pi} = 2','\phi_{pi} = 6','\phi_{pi} = 10');
L = legend([h(1) h(2)],'\phi_{\pi} = 2','\phi_{\pi} = 5');
set(L,'Location','NorthWest','Fontsize',15)

set(fig(1),'PaperOrientation','Landscape');
set(fig(1),'PaperPosition',[0 0 11 8.5]);
print(fig(1),'-depsc','RAFR_varycPHIpi.eps');

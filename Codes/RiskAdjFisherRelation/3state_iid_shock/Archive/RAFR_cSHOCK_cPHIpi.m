%--------------------------------------------------------------------------
% File Name: RAFR_cSHOCK_cPHIpi.m
% Author: Philip Coyle
...
cSIGMA = 1;
cKAPPA = 0.02;
% cPHIpi_grid = [1.2];% 2 6 10];
cPHIpi_grid = 1.1;
cRstar = 1/400;
cSHOCK_grid = [0.05, 0.075 0.1162];
% cSHOCK_grid = 0.12;
p_m = 0.5; %Probability of being in the middle state 

for k = 1:length(cPHIpi_grid)
    cPHIpi = cPHIpi_grid(k);
    titles = {'\phi_{\pi} = 1.2', '\phi_{\pi} = 2','\phi_{\pi} = 6','\phi_{\pi} = 10'};
    %% Housekeeping
    % Set Grid Intervals
    pi_m_low = -1.5/400;
    pi_m_high = 0.5/400;
    ...
    title(titles{k},'FontSize',16,'FontWeight','Normal')
    L = legend([h(1) h(2) h(3)],'Low Shock','Baseline Shock','Large Shock');
    set(L,'Location','NorthWest','Fontsize',15)

    set(fig(k),'PaperOrientation','Landscape');
    set(fig(k),'PaperPosition',[0 0 11 8.5]);
    % print(fig(k),'-depsc','RAFR_varycSHOCK_varycPHIpi.eps');
end

...
%  Taylor Rule
tr = cRstar + cPItarg + cPHIpi*(pi_m - cPItarg);
tr_zlb = 0;
...
lb_cutoff = simplify(solve(pi_h_b - bound,pi_m),'steps',500);
dpilb_dcSHOCK = diff(lb_cutoff,cSHOCK);
cSHOCK_lb = simplify(solve(lb_cutoff - dr_dss_pi,cSHOCK),'steps',500);
...
cSHOCK_ub = simplify(solve(ub_cutoff - tr_dss_pi,cSHOCK),'steps',500);
...
[rafr_lm_exp,alpha1] = subexpr(collect(simplify(rafr_lm,'steps',1000),pi_m),'alpha1');
[rafr_l_exp,alpha2] = subexpr(collect(simplify(rafr_l,'steps',1000),pi_m),'alpha2');
% pretty(rafr_lm_exp);
% pretty(rafr_l_exp);

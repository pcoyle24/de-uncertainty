function [R,J] = eqm_mex(weights,P,S,G,O,C)
...
pf_c_zlb =  Fallcheb111(O.delbound,O.del_pts,G.del_grid,O.n1,C_Ac_zlb,C.max,C.T,C.P);    
pf_inf_zlb =  Fallcheb111(O.delbound,O.del_pts,G.del_grid,O.n1,C_Ainf_zlb,C.max,C.T,C.P); 
pf_r_zlb = ones(size(pf_r));
 ...
 for i = 1:G.nodes
     ...
     if r_temp  >= 1
         ...
         C_Ar = Fchebweights11(O.n1,O.del_pts,pf_r,C.T,C.P,C.X);
         ...
         for j = 1:length(G.e_weight)
             ...
             if r_tomorrow >= 1
                 ...
                 % Get Interpolated Values
                 c_tomorrow_zlb     =  Fallcheb111(O.delbound,1,del_tomorrow(j),O.n1,C_Ac_zlb ,C.max,C.T,C.P);
                 inf_tomorrow_zlb   =  Fallcheb111(O.delbound,1,del_tomorrow(j),O.n1,C_Ainf_zlb ,C.max,C.T,C.P);
                 % Build out other PF Values
         ...
         for j = 1:length(G.e_weight)
             ...
             if r_tomorrow >= 1
                 ...
                 % Derivatives with respect to b coefficients
                 deezlb_dbxzlb = P.beta*del_today*(drzlb_dbx(C.basis(:,i),inf_today_zlb,y_today_zlb,c_today_zlb,pi_tilde_today_zlb)*(c_tomorrow^(-P.chic)*inf_tomorrow_zlb^(-1))...

...
for i = 1:G.nodes
     ...
     r_today            = S.pi_targ/P.beta*((inf_today/S.pi_targ)^(P.phi_pi)*(y_today/S.y)^(P.phi_y));
     ...
     for j = 1:length(G.e_weight)
         ...
         basis_interp = basisinterp(O.delbound,del_tomorrow(j),C.Ac,C.T,C.P); 

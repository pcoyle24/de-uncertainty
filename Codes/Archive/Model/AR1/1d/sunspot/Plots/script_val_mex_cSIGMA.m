% --------------------------------------------------------------------------
% File Name: script_val_mex_cSIGMA.m
% Author: Philip Coyle
...
for s = length(P.sigma_grid):-1:1
    ...
    for i = 1:length(P.pi_targ)
        ...
        % Save coefficients
        save(strcat(savedir,'ChebPFs_cSIGMA',num2str(100*P.sigma),'_ps',num2str(P.Ps),'_pd',num2str(P.Pd),'.mat'),'C','O','pf','S');    
        ...
        % RSS Gap 
        inx = find (abs(G.delfine -1) < 1e-10);
        rss_gap.c_d(s) = 100*(pf.cf.c_d(inx) - S.c_d);
        rss_gap.inf_d(s) = 400*(pf.cf.inf_d(inx)-1) - 400*(S.inf_d-1);
        ...
        for k = 1:length(title_fig_s)
            ...
            grid on
            if k == 1
               h(s) = plot(G.delfine,Xs(:,k),colors{s},'LineWidth',2);
            else
                plot(G.delfine,Xs(:,k),colors{s},'LineWidth',2)
            end
...
%             set(gca,'XLim',[1-4*P.bound 1+4*P.bound],'YLim',ylim_d(k,:),'FontSize',16)
            ...
            if k == 2
                line([G.delfine(inx,1) G.delfine(inx,1)],get(gca,'YLim'),'Color','k','LineStyle','-','LineWidth',0.5)
            elseif k == 3
                if ~isempty(zlb.d)
        ...
        for k = 1:length(title_fig_d)
            ...
            grid on
            if k == 4
               h(s) = plot(G.delfine,Xd(:,k),colors{s},'LineWidth',2);
            else
                plot(G.delfine,Xd(:,k),colors{s},'LineWidth',2)
            end
            xlabel('\delta','FontSize',16)
            if k == 1
                ylabel('%Dev from ESS','FontSize',16)
...
% end
L = legend([h(1) h(2) h(3)],'\sigma_{\epsilon} = 0.15/100','\sigma_{\epsilon} = 0.25/100','\sigma_{\epsilon} = 0.4/100');
set(L,'Location','SouthWest','FontSize',12)

set(fig(1),'PaperOrientation','Landscape');
set(fig(1),'PaperPosition',[0 0 11 8.5]);
% if P.alpha == 1
%     print(fig(1),'-depsc','Pfs_SunSpot_VarycSIGMA_cALPHA1.eps');
%     print(fig(1),'-dpdf','Pfs_SunSpot_VarycSIGMA_cALPHA1.pdf');
% elseif P.alpha == 0
%     print(fig(1),'-depsc','Pfs_SunSpot_VarycSIGMA_cALPHA0.eps');
%     print(fig(1),'-dpdf','Pfs_SunSpot_VarycSIGMA_cALPHA0.pdf');
% end

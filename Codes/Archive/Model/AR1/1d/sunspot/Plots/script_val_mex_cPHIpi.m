...
global pi_yesterday
pi_yesterday       = 1;
for p = 1:length(P.phi_pi_grid)
    P.phi_pi = P.phi_pi_grid(p);
    for i = 1:length(P.pi_targ)
        ...
        disp(char('Policy Functions Calculated'))
         ...
         % Save coefficients
         save(strcat(savedir,'ChebPFs_cPHIpi',num2str(P.phi_pi),'.mat'),'C','O','pf','S');    
        ...
        rss_gap.c_d(p) = 100*(pf.cf.c_d(inx) - S.c_d);
        rss_gap.inf_d(p) = 400*(pf.cf.inf_d(inx)-1) - 400*(S.inf_d-1);

        ylim_d = [-15 5;-8 6;0 10];
        ...
        for k = 1:length(title_fig_d)
            ...
            grid on
            if k == 1
               h(p) = plot(G.delfine,Xd(:,k),colors{p},'LineWidth',2);
            else
                plot(G.delfine,Xd(:,k),colors{p},'LineWidth',2)
            end
...
%             set(gca,'XLim',[1-4*P.bound 1+4*P.bound],'FontSize',16)
            ...
            elseif k == 3
                if ~isempty(zlb.d)
                    line([G.delfine(zlb.d,1) G.delfine(zlb.d,1)],get(gca,'YLim'),'Color',colors{p},'LineStyle','--','LineWidth',0.5)
                end
            end
        end
    end
...
print(fig(1),'-depsc','Pfs_SunSpot_VarycPHipi.eps');
if P.alpha == 1
    print(fig(1),'-depsc','Pfs_SunSpot_VarycPHipi_cALPHA1.eps');
    print(fig(1),'-dpdf','Pfs_SunSpot_VarycPHipi_cALPHA1.pdf');
elseif P.alpha == 0
    print(fig(1),'-depsc','Pfs_SunSpot_VarycPHipi_cALPHA0.eps');
    print(fig(1),'-dpdf','Pfs_SunSpot_VarycPHipi_cALPHA0.pdf');
end

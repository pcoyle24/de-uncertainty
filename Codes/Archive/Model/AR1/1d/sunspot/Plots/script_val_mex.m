...
for k = 1:length(title_fig_s)
    ...
    hold on
    grid on
    plot(G.delfine,Xs(:,k),'k','LineWidth',2)
    xlabel('\delta','FontSize',16)
    title(title_fig_s{k},'FontSize',16,'FontWeight','normal')
%    set(gca,'XLim',[1-4*P.bound 1+4*P.bound],'YLim',ylim_s(i,:),'FontSize',16)
    set(gca,'XLim',[1-4*P.bound 1+4*P.bound],'FontSize',16)
    if ~isempty(zlb.s)
        line([G.delfine(zlb.s,1) G.delfine(zlb.s,1)],get(gca,'YLim'),'Color','b','LineStyle','--','LineWidth',1.5)
    end
end
...
for k = 1:length(title_fig_d)
    ...
    hold on
    grid on
    plot(G.delfine,Xd(:,k),'k','LineWidth',2)
    xlabel('\delta','FontSize',16)
    title(title_fig_d{k},'FontSize',16,'FontWeight','normal')
%    set(gca,'XLim',[1-4*P.bound 1+4*P.bound],'YLim',ylim_d(i,:),'FontSize',16)
    set(gca,'XLim',[1-4*P.bound 1+4*P.bound],'FontSize',16)
    if ~isempty(zlb.d)
        line([G.delfine(zlb.d,1) G.delfine(zlb.d,1)],get(gca,'YLim'),'Color','b','LineStyle','--','LineWidth',1.5)
    end
end

set(fig(i),'PaperOrientation','Landscape');
set(fig(i),'PaperPosition',[0 0 11 8.5]);
print(fig(i),'-depsc','Pfs_SunSpot_largecSIGMA.eps');
% print(fig(i),'-depsc',strcat('Pfs_SunSpot_cSIGMA',num2str(100*P.sigma),'.eps'));
% print(fig(i),'-depsc',strcat('Pfs_SunSpot_Ps',num2str(P.Ps),'_Pd',num2str(P.Pd),'.eps'));
% print(fig(i),'-depsc',strcat('Pfs_SunSpot_cSIGMA',num2str(100*P.sigma),'_Ps',num2str(P.Ps),'_Pd',num2str(P.Pd),'.eps'));
end

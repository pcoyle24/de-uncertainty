...
% Taylor Rule
tr = max(0,cRstar + cPHIpi*(pi_m - cPItarg));
% 'Riskless' Fisher Relation
fr = pi_m  - cPItarg + cRstar;
    ...
    while converged == 0
        ...
        if sum(i >= 0) == nstate
            ...
            if sum(i == 0) == nstate
                ...
                rafr(k) = cRstar + pi_m(k) - cPItarg + cSIGMA^(-1)*(Ey - y(mid)) + (Epi - pi(mid));
            end
...
% Make legend
L = legend([h(1) h(2) h(3) pdss1 pdss2 prss2 prss1],'Taylor Rule','Standard Fisher Relation','Risk-Adjusted Fisher Relation','DSS (TE)','DSS (DE)','RSS (TE)','RSS (DE)');
...
% print(fig(1),'-dpdfc','stylized_rouwenhorst_RAFR.pdf');
print(fig(1),'-dpdfc',strcat(savedir,'RAFR.pdf'));

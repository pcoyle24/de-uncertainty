...
cRHO = 0.8; 
cPItarg= 2/400;
cSIGMAd_grid = linspace(0,cSIGMAd_max.m-eps,51);
...
sig_y_tr = zeros(length(cSIGMAd_grid),1);
sig_y_dr = zeros(length(cSIGMAd_grid),1);
...
sig_pi_tr = zeros(length(cSIGMAd_grid),1);
sig_pi_dr = zeros(length(cSIGMAd_grid),1);
...
sig_i_tr = zeros(length(cSIGMAd_grid),1);
sig_i_dr = zeros(length(cSIGMAd_grid),1);
...
sig_elb_tr = zeros(length(cSIGMAd_grid),1);
sig_elb_dr = zeros(length(cSIGMAd_grid),1);
...
for i = 1:length(cSIGMAd_grid)
    ...
    % Get Variances
    sig_y_tr(i) = std(y_tr_sim);
    sig_pi_tr(i) = std(pi_tr_sim);
    sig_i_tr(i) = std(i_tr_sim);
    sig_elb_tr(i) = std(elb_tr_sim);
...
%     [mu_i_tr(i), mu_elb_tr(i)] = exp_val(params,i_tr,R,sim,burn,1);
    ...
    % Get Variances
    sig_y_dr(i) = std(y_dr_sim);
    sig_pi_dr(i) = std(pi_dr_sim);
    sig_i_dr(i) = std(i_dr_sim);
    sig_elb_dr(i) = std(elb_dr_sim);

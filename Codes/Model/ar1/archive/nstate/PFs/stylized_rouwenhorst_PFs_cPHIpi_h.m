...
cRstar = 1/400;
cRHO = 0.8; 
cSIGMAd_grid = [0.13/100, 0.19/100, cSIGMAd_max.h-eps];
...
L = legend([h(1) h(2) h(3)],'\sigma_{\epsilon} = 0.13/100','\sigma_{\epsilon} = 0.19/100','\sigma_{\epsilon} = \sigma_{\epsilon}^{max}');
...
% print(fig(1),'-depsc','stylized_rouwenhorst_PFs_cPHIpi_h.eps');
print(fig(1),'-depsc',strcat(savedir,'stylized_rouwenhorst_PFs_cPHIpi_h.eps'));
